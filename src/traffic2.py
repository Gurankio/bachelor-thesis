...
def main():
    ...
    def isolate(upstream: Transaction) -> Generator[Transaction, None, None]:
        ...
        # Logs
        last_period = isolate.time // period

        # Wait
        isolate.time += upstream.wait
        if (new_period := isolate.time // period) > last_period:
            isolate.remaining.append(isolate.budget)
            isolate.budget = budget
            last_period = new_period

        # Delay
        if isolate.budget <= 0:
            period_left = (last_period + 1) * period - isolate.time
            isolate.delays.append(delay := period_left)
        else:
            isolate.delays.append(delay := 0)
    ...
    ber = (buffer_ends - buffer_starts - (traffic_waits + traffic_lengths)).mean()
    print(f'[blue   ]E[R_i] (buffer  )[/]={ber!s}')
    ier = (isolate_ends - isolate_starts - (traffic_waits + traffic_lengths)).mean()
    print(f'[magenta]E[R_i] (isolate )[/]={ier!s}')
    ...
    # states_freq /= np.arange(1, len(states_freq) + 1)[:, None]
